...
describe('strategic strike', () => {
  it('enforces the late-game strategic-infrastructure investment', () => {
    expect(BUILDINGS.ordnance).toMatchObject({
      ...
      cost: { funds: 2_500, stone: 450, metal: 350 }, buildWork: 12, buildTimeHours: 12,
    });
    expect(BUILDINGS.missileSite).toMatchObject({
      label: 'Missile Site', kind: 'military',
      cost: { funds: 5_000, stone: 900, metal: 900 }, buildWork: 27, buildTimeHours: 27,
    });
    ...
    for (const buildingId of ['ordnance', 'missileSite'] as const) {
      ...
      // Missile Site also gates on Hybrid technology Level 8 and phase 3 —
      // this test is about cost gating, so clear those gates up front.
      s.state.countries[SPAIN].technologies = { ...initialTechnologyLevels(), hybrid: 8 };
